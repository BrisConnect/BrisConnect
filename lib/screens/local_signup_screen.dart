...
class _LocalSignUpScreenState extends State<LocalSignUpScreen> {
  ...
  Future<void> _register() async {
    ...
    if (!registered) {
      final reason =
          LocalAuth.lastErrorMessage ?? 'Could not create Local account.';
    ...
    // Automatically send a login code so the user can sign in immediately
    // after registering. Default to email while SMS credentials are broken.
    final sendResult = await EmailCodeAuthService.sendCode(
      email: email,
      userType: 'local',
      method: 'email',
    );
    ...
    final statusMessage = sendResult == SendCodeResult.sent
        ? 'Account created! A sign-in code has been sent to your email.'
        : EmailCodeAuthService.lastErrorMessage ??
            'Account created! Use the button below to request a sign-in code.';
    final statusType = sendResult == SendCodeResult.sent
        ? InlineStatusType.success
        : InlineStatusType.info;
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          DropdownButtonFormField<String>(
                            ...
                            validator: (v) =>
                                v == null ? 'Please select your suburb' : null,
                          ),
                          const SizedBox(height: 14),
