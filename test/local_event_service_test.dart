...
void main() {
  group('LocalEventService', () {
    test('watchSubmittedEvents only returns events created by this local',
        () async {
      ...
      expect(events.first.category, 'Community');
    });

    test('watchSubmittedEvents emits status updates within two seconds',
        () async {
      final FakeFirebaseFirestore firestore = FakeFirebaseFirestore();
    ...
    test('updateSubmittedEvent resets edited events to pending status',
        () async {
      final FakeFirebaseFirestore firestore = FakeFirebaseFirestore();
      ...
      final snapshot =
          await firestore.collection('events').doc('event-2').get();
    ...
    test('deleteSubmittedEvent removes an event owned by the local user',
        () async {
      final FakeFirebaseFirestore firestore = FakeFirebaseFirestore();
      ...
      final LocalEventService service = LocalEventService(firestore: firestore);
      final didDelete = await service.deleteSubmittedEvent(
        eventId: 'event-3',
        localEmail: 'local@brisconnect.com',
      );

      final snapshot =
          await firestore.collection('events').doc('event-3').get();
