/**
 * Seed script for web app data using Firestore REST API.
 * No authentication required - uses public write access via security rules.
 *
 * Usage:
 *   cd functions
 *   node seed_web_app_data_rest.js
 */
...
function makeRequest(method, path, data = null) {
  return new Promise((resolve, reject) => {
    const url = new URL(`${BASE_URL}${path}`, "https://firestore.googleapis.com");
...
async function seedDatabase() {
  try {
    console.log("🌱 Starting database seeding via REST API...\n");
    ...
    for (let i = 0; i < categories.length; i++) {
      try {
        await makeRequest(
          "POST",
          "/event_categories",
          createDocument(categories[i])
        );
        process.stdout.write(".");
      } catch (error) {
        console.error(`\n❌ Error adding category ${i}: ${error.message}`);
      }
    }
    ...
    for (let i = 0; i < events.length; i++) {
      try {
        await makeRequest(
          "POST",
          "/events",
          createDocument(events[i])
        );
        process.stdout.write(".");
      } catch (error) {
        console.error(`\n❌ Error adding event ${i}: ${error.message}`);
      }
    }
    ...
    for (let i = 0; i < attractions.length; i++) {
      try {
        await makeRequest(
          "POST",
          "/approved_attractions",
          createDocument(attractions[i])
        );
        process.stdout.write(".");
      } catch (error) {
        console.error(`\n❌ Error adding attraction ${i}: ${error.message}`);
      }
    }
