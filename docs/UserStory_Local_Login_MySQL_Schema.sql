-- User Story: Local user secure login and local portal access
-- Target: MySQL 8+
...
CREATE TABLE auth_users (
  ...
  account_type ENUM('admin','local','visitor') NOT NULL,
  ...
  KEY idx_auth_users_type_status (account_type, status)
) ENGINE=InnoDB;
...
CREATE TABLE local_profiles (
  ...
  portal_preferences_json JSON NULL,
  created_at DATETIME(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3),
  updated_at DATETIME(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3) ON UPDATE CURRENT_TIMESTAMP(3),
  CONSTRAINT fk_local_profiles_user
    FOREIGN KEY (user_id) REFERENCES auth_users(id)
    ON DELETE CASCADE ON UPDATE CASCADE,
  KEY idx_local_profiles_approval (approval_status)
) ENGINE=InnoDB;
...
CREATE TABLE auth_sessions (
  ...
  UNIQUE KEY uq_auth_sessions_restore_hash (restore_key_hash),
  KEY idx_auth_sessions_lookup (user_id, is_revoked, expires_at),
  KEY idx_auth_sessions_device (device_id, is_revoked)
) ENGINE=InnoDB;

CREATE TABLE login_attempts (
  id BIGINT UNSIGNED AUTO_INCREMENT PRIMARY KEY,
  email_attempted VARCHAR(255) NOT NULL,
  account_type_attempted ENUM('admin','local','visitor') NOT NULL,
  user_id BIGINT UNSIGNED NULL,
  was_successful TINYINT(1) NOT NULL,
  failure_reason ENUM('invalid_credentials','locked','disabled','wrong_role','unknown_user') NULL,
  ip_address VARCHAR(45) NULL,
  user_agent VARCHAR(512) NULL,
  ...
  KEY idx_login_attempts_type_success (account_type_attempted, was_successful, attempted_at)
) ENGINE=InnoDB;
...
-- Baseline RBAC and route restrictions for local portal.
...
VALUES
('local.portal.access', 'Access local user portal dashboard and features'),
('local.routes.access', 'Access local-only routes')
ON DUPLICATE KEY UPDATE description = VALUES(description);

INSERT INTO role_permissions (role_id, permission_id)
SELECT r.id, p.id
FROM roles r
JOIN permissions p ON p.permission_code IN ('local.portal.access', 'local.routes.access')
WHERE r.role_code = 'local_user'
...
SELECT '/local/%', p.id, '*', 1
FROM permissions p
WHERE p.permission_code = 'local.routes.access'
